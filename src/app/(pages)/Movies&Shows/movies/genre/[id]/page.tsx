...
import { getMovieGenres, getMoviesByGenre } from '@/lib/api';
...
export default async function GenreMoviesPage({ params, searchParams }: PageProps) {
    ...
    const { results: movies, totalPages, totalResults } = moviesData;
            ...
            <div className="container max-w-11/12 md:max-w-4/5 mx-auto">
                <div className="mb-8">
                    ...
                    <p className="text-gray-400 mt-2">
                        Showing {movies.length} of {totalResults.toLocaleString()} movies (Page {currentPage} of {totalPages})
                    </p>
                </div>
                    ...
                    <>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6 mb-10">
                            {movies.map((movie: any) => (
                                <Card key={movie.id} className="overflow-hidden group cursor-pointer hover:border-red-500 transition-all">
                            ...
                            <Pagination>
                                <PaginationContent>
                                    <PaginationItem>
                                        {currentPage > 1 ? (
                                            <PaginationPrevious
                                                href={`/Movies&Shows/movies/genre/${genreId}?page=${currentPage - 1}`}
                                            />
                                        ) : (
                                            <PaginationPrevious
                                                href="#"
                                    ...
                                    {pageNumbers.map((pageNum, index) => (
                                        <PaginationItem key={index}>
                                            {pageNum === 'ellipsis' ? (
                                                ...
                                                <PaginationLink
                                                    href={`/Movies&Shows/movies/genre/${genreId}?page=${pageNum}`}
