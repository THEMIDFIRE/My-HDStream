        ...
        <section className="mb-20 md:mb-28 2xl:mb-32">
            <div className="container max-w-11/12 md:max-w-4/5 mx-auto">
                <Carousel opts={{ loop: true }} plugins={[Autoplay({delay: 5000})]} setApi={setApi}>
                        ...
                        {movies.map((movie: any, index: number) => (
                            <CarouselItem key={movie.id} className='relative'>
                                <div className="relative aspect-video overflow-hidden rounded-lg">
                                    ...
                                    {/* Content */}
                                    <div className='absolute bottom-1/6 left-0 right-0 z-20 text-center px-10'>
                                        ...
                                        <div className='space-x-2 mt-8'>
                                            <Button variant={'default'} className='bg-red-500 text-white hover:bg-red-700 has-[>svg]:px-3.5 has-[>svg]:py-5'>
                                                <PlayIcon />
                                                Play Now
                                            </Button>
                                            <Button variant={'default'} size={'icon'} className='bg-black text-white hover:bg-gray-800 has-[>svg]:px-3.5 has-[>svg]:py-5'>
                    ...
                    <div className="flex justify-between items-center absolute left-0 right-0 bottom-3.5 px-10">
                        <Button variant={'default'} size={'icon'} onClick={() => api?.scrollPrev()} className='pointer-events-auto bg-black hover:bg-gray-900 text-white border rounded'>
                            <ArrowLeftIcon />
                        </Button>
                        ...
                        <Button variant={'default'} size={'icon'} onClick={() => api?.scrollNext()} className='pointer-events-auto bg-black hover:bg-gray-900 text-white border rounded'>
                            <ArrowRightIcon />
                        </Button>
                    </div>
                </Carousel>
